...
module.exports = class DingzNet extends MySimpleClass {
  ...
  constructor(homey) {
    ...
    this.#mqttClient = MqttClient.getInstance(this.homey, this.rootTopic);
  }
  ...
  async publishDeviceConfig(address) {
    ...
    await this.getDingzSwitchConfig(address)
      ...
      .catch((error) => this.logError(`publishDeviceConfig() > ${address} > ${error}`));
  }
  ...
  async getDingzSwitchConfig(address) {
    ...
    try {
      const httpAPI = new HttpAPI(this.homey, `http://${address}/api/v1/`);
