...
module.exports = class DingzNet extends MySimpleClass {
  ...
  async publishDeviceConfig(address) {
    this.logDebug('publishDeviceConfig()');

    await this.getDingzSwitchConfig(address).then((config) => this.#publishConfig(config));
  }
        ...
        data.buttons.forEach((button, number) => {
          config['buttons'] = {
            [number]: {
              ...
              device: number,
              dingzButton: !(button.mode.local || button.mode.remote || button.therm_ctrl),
            },
            ...config['buttons'],
          };
        });
